...
if __name__ == "__main__":
    try:
        ...
        # отказ — тоже след: кто, что, почему, когда (N-L2-05: попытка строителя записать вердикт видна)
        try:
            записать("REFUSED", by=next((sys.argv[i + 1] for i, a in enumerate(sys.argv) if a == "--by"), None),
                     command=" ".join(sys.argv[1:3]), reason=str(e))
        except Exception:
            pass
        print(f"ОТКАЗ: {e}")
        sys.exit(3)
