...
def sudit(nomer: str, uliki: str) -> dict:
    ...
    return {
        ...
        "личности": {
            "проверяющий": {"роль": "verifier", "модель": "codex/local (Codex app-server, ChatGPT.app)",
                             "сессии": f"{CHTENIY} отдельные пустые нити Codex (ephemeral), по одной на чтение",
                             "интерпретатор": sys.executable},
            "строитель": {"роль": "builder", "модель": "claude (Claude Code, окно VS Code)",
                          "сессия": _окно_строителя(), "право_на_вердикт": "нет — ворота отклоняют validate не от Директора"},
        },
    }


def _окно_строителя() -> str:
    """Окно исполнителя по паспорту связи — та же сессия, что получает задачи."""
    try:
        import subprocess as _sp
        r = _sp.run([sys.executable, str(Path(__file__).resolve().parent.parent / "src" / "link.py"), "windows"],
                    capture_output=True, text=True, timeout=30)
        for line in r.stdout.splitlines():
            if line.strip().startswith("chosen:"):
